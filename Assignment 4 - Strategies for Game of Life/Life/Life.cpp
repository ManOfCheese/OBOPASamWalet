...
void Life::runSimulation() {
	...
	for ( size_t i = 0; i < 25; i++ ) {
		for ( size_t j = 0; j < 25; j++ ) {
			if ( calcCells[ i ][ j ] ) {
				if ( checkForLivingNeighbors( i, j ) == minNeighborToLive || checkForLivingNeighbors( i, j ) == maxNeighborsToLive ) {
				...
				if( checkForLivingNeighbors( i, j ) == repopulationThreshold ) {
					displayCells[ i ][ j ] = true;
				}
			}
		}
